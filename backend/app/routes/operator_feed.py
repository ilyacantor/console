"""Operator Feed routes — proxy to Platform's maestra plans API."""
...
async def get_plans(
    ...
    """Proxy to Platform's GET /api/maestra/plans.

    Console calls this to populate the Operator Feed with Tier 3/4
    escalation records from the maestra_plans table.
    """
    ...
    platform_url = f"{config.PLATFORM_BASE_URL}/api/maestra/plans"
    params: dict[str, str] = {
        "tenant_id": tenant_id,
        "limit": str(limit),
        "offset": str(offset),
