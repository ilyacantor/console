...
logger = logging.getLogger("console.upload")
...
async def upload_file(
    ...
    try:
        ...
        await db.update_upload(upload_id, status=status, parse_result=parse_result)
    except Exception as exc:
        logger.error(f"Parse failed for {file.filename}: {exc}")
        await db.update_upload(
            upload_id,
            status="error",
...
async def proceed_upload(upload_id: str):
    ...
    await db.update_upload(upload_id, status="converted")
